...
int Division::funcDivisionMPC(
  ...
  if (THREE_PC) {
    ...
    for (size_t looper = 1; looper < FLOAT_PRECISION_M + 1; ++looper) {
      if (PRIMARY) {
        for (size_t i = 0; i < size; ++i)
          input_1[i] = -b[i];//这里是把分母的值付给了input_1

        funcTruncate2PC(input_1, looper, size, PARTY_A, PARTY_B);//对分母进行了移位处理
        //这里的调用，input_1是b，是分母，函数里面的形参变成a了，注意一些。
        addVectors<mpc_t>(input_1, a, input_1, size);//input_1=a(分子)+input_1（分母）
        subtractVectors<mpc_t>(input_1, varP, input_1, size);//input_1=input_1-varP=a（分子）+input_1（分母）-varP
        //对应步骤的第3步这里还没有实现左移?实现了，上面的functruncate函数不错
      }
      ////funcRELUPrime3PC(input_1, varB, size);
      GetMpcOpInner(ReluPrime)->Run3PC(input_1, varB, size);//这一步得到beta，就是VARB，这是第4步，input_1不发生改变

      // Get the required shares of y/2^i and 2^FLOAT_PRECISION_M/2^i in input_1 and input_2
      for (size_t i = 0; i < size; ++i)
        input_1[i] = b[i];//这里重新赋值，将b，也就是分母，除数，重新赋值给input——1，z的值不复存在
      ...
      if (PRIMARY) {
        ...
        subtractVectors<mpc_t>(input_1, A_one, E_one, size);//E=x-a
        subtractVectors<mpc_t>(varB, B_one, F_one, size);//F=beta-b
        subtractVectors<mpc_t>(input_2, A_two, E_two, size);//input_1+input_2=X是为什么
        subtractVectors<mpc_t>(varB, B_two, F_two, size);//这里执行的是第5步乘法

        thread* threads = new thread[2];

        threads[0] = thread(
          &OpBase_::sendFourVectors<mpc_t>, this, ref(E_one), ref(F_one), ref(E_two), ref(F_two),
          adversary(partyNum), size, size, size, size);//这里把E和F衔接起来了，作用是？
        threads[1] = thread(
        ...
        //VARD指的是当j=0的时候的结果，也就是v0
        for (size_t i = 0; i < size; ++i) {
          varD[i] = input_1[i] * F_one[i];//X0*F
          temp_one = E_one[i] * varB[i];//E*Beta
          varD[i] = varD[i] + temp_one;

          if (partyNum == PARTY_A) {
            temp_one = E_one[i] * F_one[i];//E*F
            varD[i] = varD[i] - temp_one;//-E*F*j+E*Beta+X0*F
          }
        }
        //VARQ指的是当j=的时候的结果，也就是v
        for (size_t i = 0; i < size; ++i) {
          varQ[i] = input_2[i] * F_two[i];//X1*F
          temp_two = E_two[i] * varB[i];//E*Y1=E*beta
        ...
        }//上面这个地方不了解，按理来说，应该是P0和P1各自持有不同的v，但是这里的判断条件都是P0，那么P0将独自拥有varQ和D不太理解，虽然P1也是两个但是这两个怎么说？
...
//输出安全性无法保证，计算x+y 得到结果，p0p1都可以计算出对方的值，只能说计算过程中不会知道另外一方的值。
//关键在于，对于复杂的运算，在无法得到对方数据的前提下进行运算，是需要一个个基础算子的，也就是在private input和reveal这头尾之外，其余的计算都是share的形式进行in/out
int Select1Of2::mpc_select_1_of_2(
  ...
  GetMpcOpInner(DotProduct)->Run(x_minus_y, shared_bool, res_prod, size);//res_prod=(x-y)*a
  addVectors<mpc_t>(shared_y, res_prod, shared_result, size);//shared_result=res_prod+shared_y=(x-y)*a+y
...
int XorBit::mpc_xor_bit(
  ...
  size_t size) {
  vector<mpc_t> shared_sum(size, 0);//有一说一，这个size后面的数字我一直不理解
  addVectors<mpc_t>(shared_x, shared_y, shared_sum, size);//x+y
  vector<mpc_t> shared_prod(size, 0);
  GetMpcOpInner(DotProduct)->Run(shared_x, shared_y, shared_prod, size);//x*y
  if (PRIMARY) {
    for (int i = 0; i < size; ++i) {
      shared_prod[i] = shared_prod[i] << 1;
    }
  }//2xy
  subtractVectors(shared_sum, shared_prod, shared_result, size);//x异或y=x+y-2xy
...
int ReciprocalDiv::ReciprocalDivfor2(
  ...
  if (THREE_PC) {
    ...
    /*first, because we dicide to use the iteration function,so we should make sure the postivate/negativate of the den,and then get the sign of the final result, SJJ have done this part.
    ...
    */
    ...
    if (PRIMARY) {//这里的primary的条件具体含义是什么？是代表有任何一端的用户输入嘛？
    for (int i = 0; i < size; ++i) {
    ...
    for (int i = 0; i < size; ++i) {
      ...
      result[i] = initial_exp[i] + initial_temp[i] + 0.003;//这里的数字不是十进制的，怎么实现加小数，是把0.003转化为mpc_t类型嘛？
    }//3*exp+0.003
